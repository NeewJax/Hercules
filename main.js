...
const numeroTelefone = '+5555991578395'.replace(/[^0-9]/g, '');
...
setTimeout(async () => {
  if (!conn.authState.creds.registered) {
    let codigo = await conn.requestPairingCode(numeroTelefone);
  ...
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find)
        (tmp = [os.tmpdir(), 'tmp', 'jadibts'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
    }, 30 * 1000);
  }
}
