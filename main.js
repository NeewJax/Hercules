...
global.prefix = new RegExp('^[' + (opts['prefix'] || '.').replace('^\\.') + ']')
...
console.log(chalk.yellow('🚩 Escaneie seu codigo QR o codigo expirará em 60 segundos'))
}
if (connection == 'open') {
console.log(chalk.yellow('▣──────────────────────────────···\n│\n│❧ O WHATSAPP FOI CONECTADO! ✅\n│\n▣──────────────────────────────···'))}
if (connection == 'close') {
console.log(chalk.yellow(`🚩 Conexão perdida, por favor exclua a pasta $ ${global.authFile} rescaneie seu codigo QR `))}
}
...
conn.welcome = '*╔══════════════*\n*╟❧ @subject*\n*╠══════════════*\n*╟❧ @user*\n*╟❧ Bem Vindo(a)* \n*║*\n*╟❧ Descrição do grupo:*\n\n@desc\n\n*║*\n*╟❧ Fique à vontade!!*\n*╚══════════════*'
conn.bye = '*╔══════════════*\n*╟❧ @user*\n*╟❧ acabou de sair 👋🏻* \n*╚══════════════*'
conn.spromote = '*@user foi promovido a admin!!*'
conn.sdemote = '*@user deixou de ser admin !!*'
conn.sDesc = '*A descrição do grupo foi modificada*\n\n*Descrição nova:* @desc'
conn.sSubject = '*Foi modificado o título do grupo*\n*Novo nome:* @subject'
conn.sIcon = '*Essa foto nova do grupo ta bonita!*'
conn.sRevoke = '*O link do grupo foi atualizado!!*\n*Novo link:* @revoke'
...
let bio = `🤖 Tempo ativo: ${uptime} ┃ 👑 Jax ┃`
...
_quickTest()
.then(() => conn.logger.info(`Carregando...\n`))
.catch(console.error)
