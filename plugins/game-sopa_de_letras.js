...
*PALAVRA:* \`\`\`"${PALABRA}"\`\`\`
*Você tem 3 minutos para encontrar a palavra correta!!*

*ESCREVA O NÚMERO DA LINHA E DA COLUNA DO INÍCIO DA PRIMEIRA LETRA _"${PALABRA.charAt(0)}"_ DA PALAVRA _"${PALABRA}"_ TEM _${intentos}_ TENTATIVAS!!*
...
await conn.reply(m.chat, `*@${tagUser.split("@")[0]} Está jogando caça palavras 🔠 atualmente*`, m, { mentions: [tagUser] })
return
}
if (intentos === 0) {
intentos = 3  
...
if (intentos !== 0) {
await conn.reply(m.chat, `*@${m.sender.split("@")[0]} VOCÊ TEM UM MINUTO!!* 😨`, m, { mentions: [m.sender] })
}
await new Promise((resolve) => setTimeout(resolve, 3 * 60 * 1000)) // 3 min
if (intentos !== 0) {
await conn.reply( m.chat, `*@${m.sender.split("@")[0]} Acabou o tempo!!* 😧\n\n*A palavra _"${sopaPalabra}"_ Estava em _${cambioLetra}_ da linha _${fila}_ e coluna _${columna}_*`, m, { mentions: [m.sender] })
...
await m.reply(`\`\`\`🎊 Você ganhou ${diamante} ${rpgshop.emoticon('limit')}!!\`\`\`\n\n*Parabenss!! a palavra _"${sopaPalabra}"_ Estava em _${cambioLetra}_ da linha _${fila}_ e coluna _${columna}_*`)
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
}else{
if (intentos === 1) {
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
await m.reply(`🫡 *VOCÊ ESGOTOU AS TENTATIVAS!! a palavra _"${sopaPalabra}"_ Estava em _${cambioLetra}_ da linha _${fila}_ e coluna _${columna}_*`)
return  
} else {
intentos -= 1
await m.reply(`😮‍💨 *Você errou. te restam _${intentos}_ tentativas!*${intentos === 1 ? '' : `\n*PALAVRA PARA ENCONTRAR:* \`\`\`${sopaPalabra}\`\`\``}\n\n${intentos === 1 ? `\`\`\`💡 PISTA!!\`\`\`\n*A palavra_${sopaPalabra}_ Se encontra em: _"${cambioLetra}"_*\n\n` : ''}${sopaNube}`)
}}
}}

handler.command = /^(buscarpalabra|cacapalavras|caçapalavras|sopa|soup|wordsearch|wordfind|spdeletras|spletras|sppalabras|spalabras|spdepalabras)$/i
