...
if (!effect) throw '*[❗𝐈𝐍𝐅𝐎❗] Como usar esse comando?*\n—◉ _#logo (efecto) (texto)_\n*Exemplo:*\n—◉ _#logo 3d-deep-sea-metal Mystic_\n\n*[❗] Quando eu digo a eles que um texto é necessário, o uso seria:*\n—◉ _#logo (efecto) (texto1|texto2)_\n*Exemplo:*\n—◉ _#logo Wolf-Logo-Galaxy Mystic|Bot_\n\n*<Lista de efeitos/>*\n\n° ඬ⃟📝 #logo ' + effects.map(v => v.title).join('\n° ඬ⃟📝 #logo ')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*[❗𝐈𝐍𝐅𝐎❗] O efeito ${effect} Não está na lista de efeitos*`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] O efeito ${effect} Não está na lista de efeitos*` : `*[❗𝐈𝐍𝐅𝐎❗] O uso correto do comando é ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `*Toma sua imagem personalizada!!*\n*Efeito: ${effect}*`, m)
}
handler.help = ['logos']
handler.tags = ['nulis']
