...
if (user.health < 80) return conn.sendButton(m.chat,`${htki} Baixa saúde ${htka}`,
`Tua saúde 💔 está abaixo de *80!!*
Por favor, se cure primeiro para se aventurar de novo`.trim(), imgr + 'MÁ SAÚDE', [
[`❤️ CURA-ME`, `${usedPrefix}heal`]], fkontak, m)
    
if (new Date - user.lastadventure <= cooldown) return conn.sendButton(m.chat, `${htki} Descansando ${htka}`,
`Você já se aventurou, por favor espere até o final do descanso

⏱️ ${timers.toTimeString()}`.trim(), imgr + 'DESCANSANDO', [
[`🔔 Reivindicação diária`, `${usedPrefix}daily`]], fkontak, m) 
    
const rewards = reward(user)
let text = `🛫 Você está se aventurando em  *» ${kt[1][0].name}*

${cmenut}
${cmenub} *ID:* ${kt[1][0].id}
${cmenub} *Cidade:* ${kt[1][0].capitalCity}
${cmenub} *Longitude:* ${kt[1][0].longitude}
${cmenub} *Latitude:* ${kt[1][0].latitude}
${cmenuf}
...
conn.reply(m.chat, '*[❗𝐈𝐍𝐅𝐎❗] Ocorreu um erro, tente novamente, com certeza a api não gerou a imagem*', m)
}}
