...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `Texto não fornecido. Use ${usedPrefix + command} <texto>`;
    ...
    if (size >= limit_a2) {  
    await conn.sendMessage(m.chat, {text: `O áudio excede o limite de tamanho permitido. Tamanho: ${size} MB`}, {quoted: m});
    return;    
    }     
    ...
    if (size2 >= limit2) {  
    await conn.sendMessage(m.chat, {text: `O vídeo excede o limite de tamanho permitido. Tamanho: ${size2} MB`}, {quoted: m});
    return;    
    }     
    ...
    throw 'Erro ao processar o vídeo';    
    }
  }
};
