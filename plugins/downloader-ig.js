...
import('@xct007/frieren-scraper')
  ...
  .catch((error) => {
    console.error('Não foi possível importar "@xct007/frieren-scraper".');
  });

const handler = async (m, { conn, args, command, usedPrefix }) => {
  if (!args[0]) throw `Insira uma URL do Instagram como argumento. Exemplo: _${usedPrefix + command} https://www.instagram.com/reel/C8sWV3Nx_GZ/?igsh=MWZoeTY2cW01Nzg1bQ==`;
  await m.reply('Aguarde...');
  ...
  try {
    ...
    if (json.success && json.data.length > 0) {
      ...
      throw new Error('Falha na API personalizada');
    }
  } catch (err) {
    ...
    try {
      ...
      for (let i = 0; i < img.length; i++) {
        ...
        if (!bufferInfo) throw new Error('Arquivo inválido');
      ...
      try {
        ...
        for (const urRRl of datTa) {
          ...
          const tXXxt = `Aqui está o seu download: _${shortUrRRl}_`.trim();
          await conn.sendFile(m.chat, urRRl.url, 'video.mp4', tXXxt, m);
        ...
        try {
          ...
          const txt2 = `Aqui está o seu download: _${shortUrl2}_`.trim();
          await conn.sendFile(m.chat, resultss.url_list[0], 'video.mp4', txt2, m);
        } catch (err) {
          console.error('Erro no instagramGetUrl:', err.message);
          try {
            const resultssss = await instagramdl(args[0]);
            const shortUrl3 = await (await fetch(`https://tinyurl.com/api-create.php?url=${args[0]}`)).text();
            const txt4 = `Aqui está o seu download: _${shortUrl3}_`.trim();
            ...
            try {
              ...
              const txt1 = `Aqui está o seu download: _${shortUrl1}_`.trim();
              await conn.sendFile(m.chat, videoig, 'video.mp4', txt1, m);
            } catch (err) {
              console.error('Erro na API lolhuman:', err.message);
              throw 'Não foi possível realizar o download do vídeo do Instagram.';
            }
          }
        }
      }
