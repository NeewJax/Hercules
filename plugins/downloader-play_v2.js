...
if (!text) throw `[❗𝐈𝐍𝐅𝐎❗] Nome da canção faltando, por favor insira o comando mas o nome/titulo ou o link de alguma canção ou vídeo do youtube\n\n*—◉ Exemplo:\n#play.1 Good Feeling - Flo Rida*`
try {
if (command == 'play.1') {
conn.reply(m.chat, `*_⏳Seu áudio está sendo processado...⏳_*`, m) 
try {  
...
if (command == 'play.2') {
conn.reply(m.chat, `*_⏳Seu vídeo está sendo processado...⏳_*`, m)    
try {   
...
let res = await fetch(`https://api.lolhuman.xyz/api/ytplay2?apikey=${lolkeysapi}&query=${text}`)    
let json = await res.json()
await conn.sendFile(m.chat, json.result.video, 'error.mp4', `_THE HERCULES BOT_`, m)}}
} catch (e) {
m.reply('*[❗𝐈𝐍𝐅𝐎❗] Erro, você poderia tentar novamente? :( *')
}}
