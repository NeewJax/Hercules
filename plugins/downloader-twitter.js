...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) throw `Por favor, forneça um link do Twitter. Exemplo: _${usedPrefix + command} https://twitter.com/auronplay/status/1586487664274206720?s=20&t=3snvkvwGUIez5iWYQAehpw`;
  if (enviando) return;
  enviando = true;

  try {
    await conn.sendMessage(m.chat, { text: 'Por favor, aguarde...' }, { quoted: m });
    const res = await TwitterDL(text);
    if (res?.result.type === 'video') {
      const caption = res?.result.caption ? res.result.caption : 'Aqui está o seu vídeo:';
      ...
      const caption = res?.result.caption ? res.result.caption : 'Aqui está a sua foto:';
      for (let i = 0; i < res.result.media.length; i++) {
    ...
    enviando = false;
    throw 'Desculpe, houve um erro ao tentar baixar o conteúdo.';
  }
};
...
const TwitterDL = async (url) => {
  return new Promise(async (resolve, reject) => {
    ...
    if (!id)
      return resolve({
        status: "error",
        message: "ID do tweet não encontrado.",
      });
    ...
    if (response.data.code !== 200) {
      return resolve({
        status: "error",
        message: "Falha ao obter dados do tweet.",
      });
    }
