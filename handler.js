            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.reportes_solicitudes.filter(([number]) => number)) {
                        ...
                        if (data.exists)
                            await m.reply(`*[ ⚠️ Reporte de comando com falhas ⚠️ ]*\n\n*—◉ Plugin:* ${name}\n*—◉ Usuário:* ${m.sender}\n*—◉ Comando:* ${m.text}\n\n*—◉ Erro:*\n\`\`\`${format(e)}\`\`\`\n\n*[❗] Reportado ao criador do bot para ele dar uma solução, você pode usar o comando #reporte*`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
...
if (m.text && user.banned && !isROwner) {
  ...
  if (user.bannedMessageCount < 3) {
    ...
    const messageText = `❰ ⚠️ ❱ *VOCÊ ESTÁ BANIDO* ❰ ⚠️ ❱\nAviso ${messageNumber}/3 (${messageNumber} de 3)${user.bannedReason ? `\n*Motivo:* *${user.bannedReason}*` : ''}
*👉 Você pode entrar em contato com o dono do Bot se achar que é um erro (tenha prova) para tratar do motivo do seu banimento*
👉 wa.me/559891191467
`.trim();
...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  if (user.commandCount === 5) {
    ...
    if (remainingTime > 0) {
      const messageText = `*[ ⚠ ] Espera ${remainingTime} segundos antes de usar outro comando*`
      m.reply(messageText)
      return
    } else {
      user.commandCount = 0
  ...
  user.commandCount = 1
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] Seus diamantes acabaram, você pode comprar mais usando o comando ${usedPrefix}buy <cantidad>*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] Requer o nível ${plugin.level} para usar esse comando. Teu nível é ${_user.level}*`, m)
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    const isBotAdminNn = botTt2?.admin === "admin" || false
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*Sem descrição*') :
...
this.sendMessage(id, { text: `*[❗] @${user.split('@')[0]} Números arábicos ou raros não são permitidos neste grupo, então você será removido do grupo*`, mentions: [user] }, { quoted: fkontak2 });          
return    
}    
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '').replace('@subject', groupUpdate.subject)
...
export async function callUpdate(callUpdate) {
    ...
    if (nk.isGroup == false) {
    if (nk.status == "offer") {
    let callmsg = await this.reply(nk.from, `Oii *@${nk.from.split('@')[0]}*, as ${nk.isVideo ? 'video-chamadas' : 'chamadas'} não são permitidos, você será bloqueado.\n-\nSe você ligou acidentalmente, entre em contato com meu criador para desbloquear você!`, false, { mentions: [nk.from] })
    //let data = global.owner.filter(([id, isCreator]) => id && isCreator)
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝑵𝒆𝒆𝒘𝑱𝒂𝒙 👑;;;\nFN:𝑵𝒆𝒆𝒘𝑱𝒂𝒙 👑\nORG:𝑵𝒆𝒆𝒘𝑱𝒂𝒙 👑\nTITLE:\nitem1.TEL;waid=5219992095479:+521 999 209 5479\nitem1.X-ABLabel:𝑵𝒆𝒆𝒘𝑱𝒂𝒙 👑\nX-WA-BIZ-DESCRIPTION:[❗] Diga sobre coisas importantes!\nX-WA-BIZ-NAME:𝑵𝒆𝒆𝒘𝑱𝒂𝒙 👑\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: '𝑵𝒆𝒆𝒘𝑱𝒂𝒙 👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
*■ Enviando a mensagem...*
*■ Para desativar esse recurso, digite o comando:*
*—◉ #disable antidelete*
*—◉ #enable delete*
━━━━⬣  𝘼𝙉𝙏𝙄 𝘿𝙀𝙇𝙀𝙏𝙀  ⬣━━━━
`.trim(), msg, {
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando só pode ser usado pelo proprietário do Bot*',
        owner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando só pode ser usado pelo proprietário do Bot*',
        mods: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando só pode ser utilizado por moderadores e o proprietário do Bot*',
        premium: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando só pode ser utilizado por usuário premium e o proprietário do Bot*',
        group: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando só pode ser usado em grupos*',
        private: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando só pode ser utilizado no chat privado do Bot*',
        admin: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando só pode ser utilizado por admins do grupo*',
        botAdmin: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Para que você possa usar esse comando é necessário que o bot seja admin. Por favor, me dê admin!*',
        unreg: '*[ 🛑 Opaa! Você não está registrado 🛑 ]*\n\n*—◉ para usar esse comando você deve se registrar, use o comando*\n*➣ #verificar nome.idade*',
        restrict: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓𝐀 ⚠️ ] Este comando está restrigido/desativado pelo proprietário do Bot!*'
    }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: '𝙏𝙝𝙚 𝙃𝙚𝙧𝙘𝙪𝙡𝙚𝙨 𝘽𝙤𝙩', thumbnail: imagen1, sourceUrl: 'https://github.com/NeewJax/Hercules-Bot' }}}}, aa)
